...
3. **Resolution:**
   ...
   - **If NOT FOUND in any location:** Create the `.agents/instructions/` folder (or `instructions/` at root) and initialize a new `memory.instructions.md` file inside it using the **Initial Memory Template** below.
...
- **Hardcoding paths:** Never assume the memory file is always in `.agents/instructions/`. Always run the Discovery Protocol (Step 0 fast path, then Step 1 fallback) first.
